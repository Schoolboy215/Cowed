	...
	water
		...
		Enter(atom/movable/A)
			...
			if(type != /turf/water || istype(A, /obj/drawbridge/drawbridge) || locate(/obj/glass_plate, src))
				return ..()
			if(istype(A, /projectile) || istype(A, /obj/cannonball) || istype(A, /obj/Electrozap) || istype(A, /obj/Fireball) || istype(A, /obj/Iceball)) return ..()
	...
	trapdoor
		...
		attack_hand(mob/M)
			if(M.inHand(/item/weapon/axe))
				...
				if(--buildinghealth > 0)
					send_message(hearers(M), "<tt>[M.name] chops the trap door with [M.gender == FEMALE ? "her":"his"] axe!</tt>", 1)
				else
					send_message(hearers(M), "<tt>[M.name] cuts the trap door down!</tt>", 1)
					new/turf/hole(src)
			else return ..()
		verb
			Knock()
