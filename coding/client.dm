atom/movable/pixel_step_size = 4
client
	preload_rsc = "http://www.cowed.org/downloads/rsc_2.zip"
	pixel_step_size = 6
	var
		list/windows
		verbose_logout = 1
	...
	New()
		clients += src
		global.admin.GetBYONDKey(key) //cache the BYOND key (if we haven't already)
		...
		CheckAdmin()
		if(key)
			player = global.admin.GetPlayer(ckey)
			if(!player)
				player = global.admin.CreatePlayer(src)

			player.Login(src)
		else return

		if(!admin && ("max_players" in settings) && settings["max_players"] != -1)
			var/count = 0
			for(var/client/C) if(C.key) count++
			if(count > settings["max_players"])
				if(settings["max_players"] > 0)
					send_message(src, "The server has reached its maximum player limit of [settings["max_players"]] players. Please try again later.", 3)
				else
		...
		spawn(6) if(src)
			if(!(ckey in developers) && verbose_logout)
	...
	Del()
		clients -= src
		if(admin)
			for(var/admin_pm/pm in admin_pms) pm.removeAdmin(src)
	...
	AllowUpload(filename, filelength)
		...
		if(ckey != "androiddata" && filelength > 1048576) return 0
	...
	Click(atom/O, location, control, params)
		...
		if(control == "admin/viewscreen.lstPlayers")
			var/admin_viewscreen/C = global.admin.admin_options[usr.ckey]["admin_viewscreen"]
			if(C) C.Select(O, mob)
			return
		if(!map_editor) return ..()
	...
	Move(loc, dir, forced = 0, running = 0)
		...
		if(mob._c_speed != mob.speed)
			...
			mob._speed = (1 / (mob.speed / 100)) * 1.3
