	...
	container
		...
		Click(location, control, params)
			if(control == "admin/classban.grdKingdoms" || control == "admin/classban.grdBranches")
				...
				if(params["right"])
					var/player/P = admin.admin_options[usr.ckey]["jobban_player"]
					P.ToggleJobBan(class_id)
				else
					if(parent_type == /character_handling/container)
						admin.admin_options[usr.ckey]["jobban_kingdom"] = src
					else
						admin.admin_options[usr.ckey]["jobban_branch"] = src
				admin.UpdateClassBanList(usr)
				return
	...
	class
		...
		Click(location, control, params)
			...
			if(src.class_id && P.class_id_ban && (src.class_id in P.class_id_ban))
				...
				return
			if(!isnull(rp_points) && rp_points > P.GetVariable("score_rppoints"))
		...
		CanSelect(mob/character_handling/M)
			var/player/P = M.client.player
			if(P && P.class_id_ban && (src.class_id in P.class_id_ban)) return 0
			if(!isnull(rp_points) && rp_points > P.GetVariable("score_rppoints")) return 0
			return 1
		proc
		...
		special
			...
			necromancer
				...
				Selected(mob/M)
					...
					M.contents += new/item/armour/body/necromancer(M)
