...
mob
	anchored = 0
	var
		state = "alive" //possible values: alive, ghost, ghost-i, zombie, skeleton
	...
	proc
		...
		make_wizard(color)
			...
			//src.learn_spell(/spell/create_orb, 0)
			src.mage = 1
	...
	verb
		...
		Medals()
			...
			if(!L || !L.len)
				...
				for(var/medal in L) usr << "<tt>\t [medal2text(medal)]</tt>"
		...
		say(msg as text)
			...
			admin.AddChatLog(src.key, src.name, html_encode(msg), "say")
		OOC(msg as text)
			set hidden = 1
			if(!client || global.admin.IsMuted(client))
				send_message(usr, "You're muted!", 3)
				return
			...
			admin.AddChatLog(src.key, src.name, html_encode(msg), "ooc")
		emote(T as text)
			...
			admin.AddChatLog(src.key, src.name, html_encode(T), "emote")
					...
					if(other)
						...
						if(ismob(other.loc))
							...
							for(var/mob/N in hearers(2, other))
								if(!(N in L) && !(N in hearers)) L += N
			for(var/mob/N in L) N.show_message(., 2)
			admin.AddChatLog(src.key, src.name, html_encode(T), "whisper")
		reboot_check_time()
		...
		loot(mob/M in view(1, src))
			...
			spawn(rand(40, 100))
				...
				if(fail)
					...
					admin.AddChatLog(src.key, src.name, "Looted [M.key]/[M.name] (failed to take [I])", "event")
					admin.AddChatLog(M.key, M.name, "Looted by [src.key]/[src.name] (failed to take [I])", "event")
					if(issleeping)
						toggle_sleep(0)
						if(prob(25))
					...
					admin.AddChatLog(src.key, src.name, "Looted [M.key]/[M.name] ([I] was taken)", "event")
					admin.AddChatLog(M.key, M.name, "Looted by [src.key]/[src.name] ([I] was taken)", "event")
	...
	Logout()
		...
		else if(istype(using, /admin_viewscreen))
	...
	Move(newloc, newdir, forced = 0)
		...
		if(whopull)
			if(shackled || istype(src.loc, /atom/movable) || get_dist(src, whopull) > 1 || whopull == src || !isturf(whopull.loc))
				whopull = null
	...
	MouseDrop(obj/chest/O)
		if(!CheckGhost() && istype(O) && O.icon_state == "chest open" && (usr in range(1, O)) && (src in range(1, usr)) && HP > 0)
			for(var/mob/N in hearers(src))
...
/*		if(usr.passworded == 1)
	...
	/*	berry_effect(state)
		...
		attack(mob/M)
			...
			switch(attackmode)
				...
				if("Attack")
					...
					admin.AddChatLog(src.key, src.name, "Attacked [M.key]/[M.name]", "event")
					admin.AddChatLog(M.key, M.name, "Attacked by [src.key]/[src.name]", "event")
				...
				if("Blunt")
					...
					admin.AddChatLog(src.key, src.name, "Attacked [M.key]/[M.name] (non-deadly)", "event")
					admin.AddChatLog(M.key, M.name, "Attacked by [src.key]/[src.name] (non-deadly)", "event")
		...
		checkdead(mob/M)
			...
			admin.AddChatLog(src.key, src.name, "I have been killed[istype(M) && M.client ? " by [M.key]/[M.name]":]!", "event")
			M.show_message("<tt>You fall down dead. [global.settings["respawn_time"] > 0 ? "You may respawn in [round((global.settings["respawn_time"] / 10) / 60)] minute\s." : "You may now observe the game and see what other players are up to."]</tt>")
	...
	observer
		...
		New(loc, force_am = 0)
			. = ..()
			if(!force_am && global.settings["respawn_time"])
				ActionLock("allow_abandon", global.settings["respawn_time"])
				spawn(global.settings["respawn_time"])
					if(abandon_mob) src.show_message("<tt>You may now abandon mob.</tt>")
			else if(force_am) override_am = 1
		Login()
		...
		verb
			abandon_mob()
				if(!override_am && !global.settings["respawn_time"])
